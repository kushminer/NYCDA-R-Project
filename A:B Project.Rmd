...
library('devtools')
library('ropensci/plotly')
library('plotly')

# Filter data for each group
group1_data <- combined_data %>%
  filter(group == "average_bidding")

group2_data <- combined_data %>%
  filter(group == "maximum_bidding")

# Create a function to generate funnel plot data
create_funnel_data <- function(data) {
  # Calculate the percentage of each variable compared to the previous variable
  funnel_data <- data %>%
    group_by(variable) %>%
    summarize(sum = sum(value)) %>%
    mutate(percent = cumsum(sum) / sum(sum))

  # Reverse the order of the data to match the desired funnel order
  funnel_data <- funnel_data[nrow(funnel_data):1,]

  return(funnel_data)
}

# Generate funnel plot data for each group
group1_funnel_data <- create_funnel_data(group1_data)
group2_funnel_data <- create_funnel_data(group2_data)

# Create the funnel plots
group1_funnel_plot <- plot_ly(
  type = "funnelarea",
  values = group1_funnel_data$percent,
  text = rev(group1_funnel_data$variable),
  ...
  opacity = 0.65,
  name = "Average Bidding"
)

group2_funnel_plot <- plot_ly(
  type = "funnelarea",
  values = group2_funnel_data$sum,
  text = rev(group2_funnel_data$variable),
  marker = list(colors = c("deepskyblue", "lightsalmon", "tan", "teal", "silver"),
                line = list(color = c("wheat", "wheat", "blue", "wheat", "wheat"), width = c(0, 1, 5, 0, 4))),
  textfont = list(family = "Old Standard TT, serif", size = 13, color = "black"),
  opacity = 0.65,
  name = "Maximum Bidding"
)

# Display the funnel plots
group1_funnel_plot
group2_funnel_plot
