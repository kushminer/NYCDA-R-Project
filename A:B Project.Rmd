...
```{text, Project Goal}
  ...
  Our company recently introduced a new bidding type, “average bidding”,
as an alternative to its existing bidding type, “maximum bidding”. 
 
  One of our clients wants to test this new feature and A/B test to understand if average bidding brings more conversions than maximum bidding.
 
  The A/B test ran for 1 month and the client now expects you to analyze and present the results of this A/B test.
  
-- -- -- 

average will not go above the max
maxmimum bid is you will max bid
...
compare_bidding_by_day_and_variable <- function(date, variable, average_bidding, maximum_bidding) {
  ...
  # Comparison Plot
  ggplot(combined_data, aes(x = `Campaign Name`, y = as.numeric(combined_data[,variable]), fill = `Campaign Name`)) +
    geom_bar(stat = "identity") +
    labs(x = "Campaign Name", y = variable) +
    ggtitle(paste0("Comparison of ", variable, " between Average Bidding and Maximum Bidding")) +
...
```
***Adjust to Even days of Week, Correct Code***

```{r, DataFrames by Day of Week}
...
# Conversion Rate Plot by Variable
...
geom_col() +
geom_text(aes(label = round(ratio, 4)), position = position_dodge(width = 0.9), vjust = 2.5) +
labs(x = "", y = "") +
ggtitle("Conversion Rates") +
facet_wrap(~ variable, ncol = n_cols, scales = "free_y") +
theme(plot.title = element_text(hjust = 0.5))
}
...
n1 = NULL # sample size of group 1 (control)
n2 = 29   # sample size of group 2 (test)
delta = delta    # effect size, difference in means between the two groups
alpha = 0.05     # significance level
power = 0.95     # desired power (probability of correctly detecting a difference between two groups)
